function [pulse_samps, channel_order] = unique_isi_sequence(duration,min_delay,max_delay,n_channels, sample_rate)
...
pulse_samps = linspace( min_delay,...
                        max_delay,...
                        floor(n_samples/mean([min_delay,max_delay])))';

% Shuffle delays 
pulse_samps = pulse_samps( randperm( numel(pulse_samps)));

% Sum to create cumulative vector of pulse times:
pulse_samps = cumsum(pulse_samps);

%crop the vector to duration:
pulse_samps = pulse_samps(pulse_samps<=n_samples);
pulse_samps = round(pulse_samps);

%present warning if range of specified delays won't result in unique ISIs:
if length(unique(diff(floor(pulse_samps))))<length(pulse_samps)-1,
...
remainder = rem(length(pulse_samps),n_channels);
%remove these extra pulses:
pulse_samps(end-remainder+1:end) = [];

% generate a randomizer across channels:
channel_order = repmat([1:n_channels]',length(pulse_samps)/n_channels,1);
