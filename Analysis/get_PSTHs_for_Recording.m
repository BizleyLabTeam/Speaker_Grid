function get_PSTHs_for_Recording(tank, block, opt)
%
...
%
% INPUT:
%   - tank: path to directory containing recordings
%   - block: directory containing h5 files
...
%
% Stephen Town
%   - Branched from get_PSTHs_for_Block.m - 14 May 2021

try
          
    % Get block path if not entered
    if nargin == 0 
       file_path = uigetdir('G:\UCL_Behaving', 'SELECT BLOCK PATH'); 
    ...
    if nargin < 3               % Defaults unless user says otherwise                 
        opt.save = true;
        opt.overwrite = true;
        opt.save_root = 'C:\Analysis\Behavioral_Recording';
    ...
    B = align_event_times(file_path, aet_opt);              
    opt.stimTimes = B.MCS_Time;
    
    if all(isnan(opt.stimTimes)), return; end
    opt.tlim = [min(opt.stimTimes)-5, max(opt.stimTimes)+5];  % Time limit for cleaning (for accelerating code)
    ...
    for i = 1 : numel(h5_files)
        ...
        if opt.save   
            ...
            stim_fpath = fullfile(save_path, B.Properties.Description);     % Stimulus info with matched times
            writetable(B, stim_fpath, 'delimiter', ',');
        ...
        if opt.draw.psth    % Draw PSTH
            fig.PSTH = draw_psth( h5_files(i).name, spike_times, opt.stimTimes, chan_map);
        ...
        if opt.draw.raster  % Draw Raster                        
            ...
            for chan = 1 : numel(spike_times)                                                            
                ...
                taso = bsxfun(@minus, transpose(spike_times{chan}), transpose(opt.stimTimes));
